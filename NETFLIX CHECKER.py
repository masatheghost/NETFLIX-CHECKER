import os, re, time, random, threading, requests, easygui, datefinder, pyfiglet
...
class Netflixer:
    ...
    def ui(self):
        os.system("cls && title [NETFLIX CHECKER] - Made by MASA THE GHOST")
        logo = pyfiglet.figlet_format('MASA THE GHOST')
        print(logo)
    ...
    def updateTitle(self):
        while True:
            ...
            os.system(
                f"title [NETFLIX CHECKER] - Hits: {self.hits} ^| Bad: {self.bad} ^| Retries: {self.retries} ^| CPM: {self.cpm} ^| Threads: {threading.active_count() - 3} ^| Time elapsed: {elapsed}"
            )
            time.sleep(0.4)
    ...
    def getProxies(self):
        try:
            ...
            path = easygui.fileopenbox(
                default="*.txt",
                filetypes=["*.txt"],
                title="NETFLIX CHECKER - Select proxy",
                multiple=False,
            )
            try:
    ...
    def getCombos(self):
        try:
            ...
            path = easygui.fileopenbox(
                default="*.txt",
                filetypes=["*.txt"],
                title="NETFLIX CHECKER - Select combos",
                multiple=False,
            )
            with open(path, "r", encoding="utf-8") as f:
                for l in f:
